...
- `default_report_url`：默认上报地址。应用详情页 V2 配置卡片 `report_url` 输入框旁的「使用默认值」按钮读取该值。
...
{
  "data": [
    { "key": "default_report_url", "value": "https://report.example.com/x", "updated_at": "2026-07-22T00:00:00Z" }
  ]
}
```

#### PUT /api/settings（仅 admin）
...
```json
{
  ...
  "value": "https://report.example.com/x"
}
```
